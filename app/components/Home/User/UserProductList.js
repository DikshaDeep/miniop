...
class UserProductList extends Component {
  ...
  GoToEdit = (productName, size, id) => {
    this.props.navigation.navigate(SCREENS.ADDPRODUCT, {
      type: "UserProductListEditWithoutBatchid",
      data: {
  ...
  renderAllListView = ({ item, index }) => {
    if (this.props.route.params.type == "Userlabellist") {
      return (
        <TouchableOpacity
          style={styles.card}
        ...
        >
          <View style={styles.mainView}>
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                this.GoToEdit(item.productName, item.size, item.id)
              }
            >
              <Image
                resizeMode={"contain"}
